...
public partial class MainWindow : Window
{
    ...
    private void LaunchJaasmChrome_Click(object? sender, RoutedEventArgs e)
    {
        try
        {
            if (!OperatingSystem.IsWindows())
            {
                ModBrowseStatusText.Text =
                    "One-click JAASM Chrome launch is currently implemented for Windows.";
                AppendConsole("[MOD EXTENSION] JAASM Chrome launcher is currently Windows-only.");
                return;
            }
            ...
            if (chrome is null)
            {
                ModBrowseStatusText.Text = "Google Chrome was not found.";
                AppendConsole("[MOD EXTENSION] Chrome executable not found.");
                return;
            }
            ...
            if (!Directory.Exists(bundled))
            {
                ...
                AppendConsole($"[MOD EXTENSION] Missing bundled extension directory: {bundled}");
                return;
            }
            ...
            var extensionDirectory = Path.Combine(root, "browser-extension");
            var chromeProfile = Path.Combine(root, "chrome-profile");

            CopyDirectory(bundled, extensionDirectory);
            Directory.CreateDirectory(chromeProfile);

            var search = Uri.EscapeDataString(ModBrowseSearchBox.Text?.Trim() ?? string.Empty);
            var targetUrl = string.IsNullOrWhiteSpace(search)
            ...
            var arguments =
                $"--user-data-dir={QuoteProcessArgument(chromeProfile)} " +
                $"--load-extension={QuoteProcessArgument(extensionDirectory)} " +
                "--no-first-run --no-default-browser-check " +
                QuoteProcessArgument(targetUrl);
            ...
            ModBrowseStatusText.Text =
                "JAASM Chrome launched with the mod extension loaded automatically.";

            AppendConsole(
                $"[MOD EXTENSION] Launched isolated Chrome profile with extension: {extensionDirectory}");
        }
        catch (Exception ex)
        {
            ModBrowseStatusText.Text = "Could not launch JAASM Chrome.";
            AppendConsole($"[MOD EXTENSION] JAASM Chrome launch failed: {ex.Message}");
        }
    }
